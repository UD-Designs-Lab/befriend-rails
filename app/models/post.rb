class Post < ApplicationRecord
  ...
  enum reply_status: {
         everyone: "everyone",
         people_you_know: "people_you_know"
       },
       _suffix: :can_reply
  ...
  scope :from_people_you_know,
        ->(user) { from_people_you_follow(user).or(own_posts(user)) }
  scope :consist_images, -> { where.not(image: nil) }
  scope :with_comments_from,
        ->(user) { includes(:comments).where(comments: { user_id: user.id }) }
end
