class RepostsController < ApplicationController
  before_action :set_repost, only: [:vote_like, :vote_smile, :vote_thumbs_up]
  def index
    @reposts = Repost.from_people_you_know(current_user).includes(:repostable, :user)
  end

  def create
    @repost = @repostable.reposts.build(repost_params)
    @repost.user_id = current_user.id
    return error('bad_request') if current_user.already_reposted?(@repost.repostable)

    if @repost.save
      render :new, status: :created
    else
      render json: { message: @repost.errors.full_messages }, status: :bad_request
    end
  end
